...
export interface ProductsTableProps {
  ...
  /** Manual stock adjust via POST /products/:id/stock (tracked products only). */
        ...
        <table className="w-full">
          <thead className="bg-flo-bg border-b border-flo-border">
            <tr>
              ...
              <th className="text-center p-4 text-caption font-medium text-flo-text-muted uppercase">
                Add-ons
              </th>
              ...
              <th className="text-left p-4 text-caption font-medium text-flo-text-muted uppercase">
                {t('products.columnTax')}
              </th>
              {loyaltyEnabled && (
                <th className="text-left p-4 text-caption font-medium text-flo-text-muted uppercase">
                  {t('products.columnCashback')}
                </th>
              )}
              <th className="text-center p-4 text-caption font-medium text-flo-text-muted uppercase">
                {t('products.columnStock')}
              </th>
              <th className="text-center p-4 text-caption font-medium text-flo-text-muted uppercase">
          ...
          <tbody className="divide-y divide-flo-border">
            {products.map((product) => {
              const parentCat = categories.find(
                (c) => String(c.id) === String(product.category_id || product.category?.id),
              );
              const isCategoryInactive = Boolean(parentCat && !parentCat.is_active);
              const matchedTaxCategory = taxCategories.find(
                (tc) => tc.id === product.tax_category_id,
              );
              const taxLabel = product.tax_category_id
                ? matchedTaxCategory
                  ? taxCategoryOptionLabel(matchedTaxCategory)
                ...
                <tr key={product.id} className="hover:bg-flo-bg/60">
                  <td className="p-4 max-w-[220px]">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {product.sku && (
                          <p className="text-caption text-flo-text-muted mt-0.5">
                            {t('products.skuLabel', { sku: product.sku })}
                          </p>
                        )}
                        {product.barcode && (
                          <p className="text-caption text-flo-text-muted mt-0.5 font-mono">
                  ...
                  <td className="p-4 text-small text-flo-text-secondary">
                    <div className="flex flex-col gap-0.5">
                      ...
                      {isCategoryInactive && (
                        <StatusBadge
                          variant="warning"
                          className="w-fit text-[11px] h-auto py-0.5"
                          title="Parent category is inactive; product is hidden on POS"
                        >
                  ...
                  <td className="p-4 text-right">
                    ...
                    {product.cost_price != null && product.cost_price > 0 && (
                      <p className="text-caption text-flo-text-muted">
                        Cost: {fmt(Number(product.cost_price))}
                      </p>
                    )}
                  </td>
                  <td className="p-4 text-small text-flo-text-secondary">
                    <div className="flex flex-col gap-0.5">
                      ...
                      {!product.tax_category_id && taxCategories.length > 0 && (
                        <StatusBadge
                          variant="warning"
                          className="w-fit text-[11px] h-auto py-0.5"
                          title={t('products.notTaxedTooltip')}
                        >
                          <AlertTriangle size={11} className="shrink-0" aria-hidden />
                  ...
                  {loyaltyEnabled && (
                    <td className="p-4 text-small text-flo-text-secondary">
                      {product.cb_percent === null || product.cb_percent === undefined ? (
                        <span>
                          {globalCashbackPercent}%{' '}
                          <span className="text-flo-text-muted text-caption">
                            ({t('products.cashbackGlobalBadge')})
                          </span>
                        </span>
                  ...
                  <td className="p-4 text-center">
                    {product.track_inventory ? (
                      <span
                        className={`text-small font-medium ${product.stock_quantity <= (product.low_stock_threshold || 0) ? 'text-flo-danger' : 'text-flo-text'}`}
                      >
                  ...
                  <td className="p-4 text-center">
                    ...
                    {product.is_active && isCategoryInactive && (
                      <span className="text-[10px] text-flo-warning font-medium block mt-1">
                        (Hidden on POS)
                      </span>
                    )}
                  </td>
                        ...
                        <>
                          {onAdjustStock && product.track_inventory ? (
                            <button
                              ...
                              aria-label={t('stockAdjust.title')}
