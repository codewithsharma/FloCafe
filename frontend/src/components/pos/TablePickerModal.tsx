...
const statusStyles: Record<
  ...
  {
    border: string;
    badgeVariant: 'warning' | 'info' | 'secondary' | 'default';
    badgeKey: string | null;
  }
> = {
  available: {
    border: 'border-flo-border hover:border-flo-brand-400',
  ...
  occupied: {
    ...
    badgeKey: 'pos.tableOccupied',
  },
  reserved: {
    border: 'border-flo-warning bg-flo-warning-subtle',
    badgeVariant: 'warning',
    badgeKey: 'pos.tableReserved',
  },
  cleaning: {
    border: 'border-flo-border bg-flo-bg',
    badgeVariant: 'secondary',
    badgeKey: 'pos.tableCleaning',
  },
  held: {
    border: 'border-flo-info bg-flo-info-subtle',
    badgeVariant: 'info',
    badgeKey: 'pos.tableHeld',
  },
};
...
export default function TablePickerModal({
  ...
  const handleClick = (table: Table) => {
    ...
    if (table.status === 'available' || table.status === 'reserved') {
      const customer =
        table.status === 'reserved' && table.reservation_customer_id
          ? {
              id: table.reservation_customer_id,
              name: table.reservation_customer_name ?? '',
              phone: table.reservation_customer_phone ?? '',
            }
          : null;
  ...
  return (
    ...
    >
      <DialogContent className="border-flo-border bg-flo-surface sm:max-w-lg max-h-[80vh] overflow-hidden flex flex-col gap-0 p-0">
            ...
            {tables.map((table) => {
              ...
              return (
                <button
                  ...
                  disabled={isDisabled}
                  className={`p-4 min-h-14 rounded-flo-lg border-2 text-center transition-colors relative ${
                    isSelected
                ...
                >
                  {isHeld && (
                    <StatusBadge variant="info" className="absolute top-1 right-1 h-5 text-[10px]">
                      {t('pos.tableHeld')}
                    </StatusBadge>
                  )}
                  {!isHeld && style.badgeKey && (
                    <StatusBadge
                      ...
                      className="absolute -top-2 -right-2 h-5 text-[10px]"
                    >
                      {t(style.badgeKey)}
                    </StatusBadge>
                  )}
                  ...
                  <p className="text-xs text-flo-text-secondary">
                    {t('pos.tableSeats', { count: table.capacity })}
                  </p>
        ...
        {selectedTableId && (
          <div className="flex gap-3 shrink-0 px-5 py-4 border-t border-flo-border">
            <Button
              type="button"
              variant="outline"
