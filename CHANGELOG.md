...
All notable changes to Opervia (repo/package legacy: Flo Cafe / flo-desktop) are documented here. Dates are release dates, not commit dates. Format loosely follows [Keep a Changelog](https://keepachangelog.com/).

## [Unreleased]

### Added

- Deploy/start vertical configuration via `ACTIVE_VERTICAL_ID` (`restaurant` default when unset; `retail` production composition; `retail-test` synthetic only).
...
- Pre-migration auto-backup now runs before _every_ upgrade, not just two specific hardcoded versions — an install that's been stuck for a long time and jumps through a dozen+ migrations at once is now just as protected as one applying a single routine update.
