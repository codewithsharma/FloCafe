...
Before a pending migration runs, Operavia creates a timestamped local backup. Migrations are additive and tracked with SQLite's `user_version` pragma (current schema **v83**). Use Settings → Database Tools → Backup before moving to another computer or switching distribution channels.
