...
- [x] **GUI-0005 + GUI-0006 fix + retest (2026-08-15):** `canAccessAppPath`/`getRolesForAppPath` + AuthGuard; chef kitchen nav. Evidence `FINAL-RBAC-RETEST.md`. Verdict: **GUI RBAC REGRESSION PASS**; full GUI still **PASS WITH CONDITIONS** (offline/Electron/printer/restore).
- [x] **Final RBAC GUI audit / release gate (2026-08-15):** Implementation review + unit suites + Chromium 5-role matrix (failCount 0). Evidence `FINAL-RBAC-AUDIT.md`. Verdict: **GUI RBAC AUDIT — PASS**; **FULL GUI QA — PASS WITH CONDITIONS**.
- [x] **RC real-world QA (2026-08-15):** Offline local-first + badge, print graceful fail, day-close 201/409, backup/restore PIN-blocked on e2e-as-node, Electron concurrent e2e BLOCKED. Evidence `FINAL-RC-REAL-WORLD-QA.md`. Verdict: **RELEASE CANDIDATE QA — PASS WITH CONDITIONS**.
- [x] **Production readiness audit (2026-08-15):** Secrets SAFE; unauth API 401; RBAC/money GUI green; signed RC + OPS-02 + Master-PIN DR still required. Doc `docs/qa/FINAL-PRODUCTION-READINESS-AUDIT.md`. Decision: **CONDITIONAL GO**.
- [x] **Final release hardening (2026-08-15):** P1-06 unopenable DB → `corrupt_database` recovery; JWT/CSP classified Phase C (no blind migrate); REAL dual-write residual documented; OPS-02 site drill checklist; RC checklist; hardening audit. Doc `docs/qa/FINAL-RELEASE-HARDENING-AUDIT.md`. Verdict: **CONDITIONAL GO**.
- [ ] **Next program:** unsigned→signed/notarized RC when credentials available; execute `docs/ops/OPS-02-SITE-DRILL-CHECKLIST.md`; desktop Master-PIN backup/restore drill; written JWT/CSP residual acceptance or Phase C.
