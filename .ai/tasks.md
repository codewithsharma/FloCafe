...
- [ ] **Hardening (prefer next, authorized slice only):** audit trail; reliability error handling depth; data integrity validation. POS H1 + KDS H2 + RBAC H3 + Restore/Conflict H4 closed (depth remains Hardening where noted). OPS-01 ops docs closed. OPS-02 live = NO-GO until signed RC + site checklist.- [ ] **Planned (not started):** see matrix — 86 depth, notes, combos, courses, QR ordering, recipes/BOM, PO/suppliers, floor plan/merge/seats, KDS routing/timers, print queue/retry, reports, workflow tests. Reuse shipped slices; do not rebuild.
