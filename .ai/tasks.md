...
- [ ] **R1 POS Core Completion (not started):** requires explicit authorization. See `restaurant-os-roadmap.md`.
- [ ] **Hardening (prefer next, authorized slice only):** audit trail; reliability error handling depth; data integrity validation — or fold into R1 if authorized. POS H1 + KDS H2 + RBAC H3 + Restore/Conflict H4 closed (depth remains Hardening where noted). OPS-01/02 closed. R0 closed.
