...
| **Phase 3** | **3.1–3.4 COMPLETE**; **3.5A–3.6E COMPLETE** (3.5B DEFERRED; 3.5C no safe extraction); **3.6F Cash Drawer Kick COMPLETE**. Remaining depth work TBD. Deploy/start: `ACTIVE_VERTICAL_ID` env (unset→`restaurant`; empty/unknown fail-closed); `retail` = production Retail; `retail-test` = synthetic validation only. |
...
**P1.6 pilot gates / first supervised café** (human/operational; intentionally deferred for engineering). Phase **3.6F COMPLETE** (Cash drawer kick — ESC/POS via default printer; no money-path). Continue authorized product-depth slices only; do not reopen 3.5B / extraction / P1.6.
