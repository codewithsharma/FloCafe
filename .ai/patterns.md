...
- **Boundary validation:** Zod schemas in `main/validation/` + `validateBody` middleware for untrusted HTTP input (auth first).
- **Observability:** `main/lib/logger.ts` (pino + pretty in dev) + `pino-http` / helmet / compression via `main/middleware/http-observability.ts`; OTel API spans via `main/lib/tracing.ts` (noop without collector).
- **Frontend state:** TanStack Query = server/API; Zustand = client/UI; SQLite = persistent domain (`frontend/src/lib/state-ownership.ts`).
- **i18n:** i18next namespaces under `frontend/src/locales/{lang}/`; legacy flat catalogs remain for unmigrated UI.
- Opervia module registry (**Phase 2 COMPLETE**): `main/modules/` describes capabilities; composition snapshot + GET API; soft diagnostics; `isModuleEnabled` / `isFeatureAvailable`. Restaurant side effects on Order/Payment soft-gated. POS is an orchestrator (`checkout-coordinator`). Synthetic `retail-test` never active. Final gate: `docs/03-architecture/phase-2-final-exit-gate.md`.
