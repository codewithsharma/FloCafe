...
- **Domain boundary hardening (Phase 2.7–2.8):** Inventory mutations go through `main/services/inventory.ts` inside caller `withTxn` (or service `withTxn` for adjust). Phase 2.8: every stock mutation also **INSERT**s into append-only `inventory_movements` in the same txn; never UPDATE/DELETE movements. Tax money-path discount scaling goes through `scaleItemTax*` in `main/services/tax.ts` (preserve Math.round). Prefer service ownership over scattering SQL.
