...
- **Domain boundary hardening (Phase 2.7–2.9):** Inventory owns **stock writes** via `main/services/inventory.ts` (`decrement`/`restore`/`adjust`/`applyAbsoluteStockChange`). Product create/PUT route stock through Inventory inside `withTxn`. Append-only `inventory_movements`; zero-delta skips INSERT. Reads may use `products.stock_quantity`. Tax money-path discount scaling via `scaleItemTax*` (preserve Math.round).
