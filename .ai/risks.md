...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM (writes+ledger owned; stock columns still on products); Tax MEDIUM (HTTP + snapshot contract frozen; denormalized columns + money-path orchestration remain); Product/POS/KDS still HIGH — see `extraction-readiness.md`.
