...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM (writes+ledger+history HTTP; stock columns still on products; write HTTP still product-nested); Tax MEDIUM (HTTP + snapshot contract + Product config ownership clarified in 2.13; denormalized columns + money-path orchestration remain); Product HIGH (tax config refs collocated; legacy tax_type/tax_rate); POS/KDS still HIGH — see `extraction-readiness.md`.
