...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM with ledger (v75) but stock columns still on products; Tax MEDIUM; Product/POS/KDS still HIGH — see `extraction-readiness.md`; do not package-extract prematurely.
- ⚠️ RISK: Inventory ledger starts at v75 with **no backfill** — pre-migration history unavailable; product create/PUT stock bypasses ledger until wired; refunds intentionally do not restock (money vs stock diverge).
