...
- ⚠️ RISK: Doc drift — some older architecture pages may still cite pre-v75 schema; Phase 2 truth is **exit gate + `.ai/`** (continuation through 2.17; multi-vertical runtime / packages are Phase 3).
...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM (writes+ledger+history HTTP; stock columns still on products; write HTTP still product-nested); Tax MEDIUM (HTTP + snapshot contract + Product config ownership clarified in 2.13; denormalized columns + money-path orchestration remain); Product HIGH (tax config refs collocated; legacy tax_type/tax_rate); POS MEDIUM-HIGH after 2.16 checkout seam (page still owns retry/discount UX); KDS still HIGH — see `extraction-readiness.md`. Phase 2 CONTINUATION; extraction is Phase 3.
