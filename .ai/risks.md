...
- ⚠️ RISK: Doc drift — some older architecture pages may still cite pre-v75 schema; Phase 2 truth is **exit gate + `.ai/`** (Phase 2 COMPLETE through 2.14; multi-vertical runtime / packages are Phase 3).
...
- ⚠️ RISK: Soft module registry — Express routes remain statically mounted; composition does not unload APIs (fail-closed deferred Phase 3).
...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM (writes+ledger+history HTTP; stock columns still on products; write HTTP still product-nested); Tax MEDIUM (HTTP + snapshot contract + Product config ownership clarified in 2.13; denormalized columns + money-path orchestration remain); Product HIGH (tax config refs collocated; legacy tax_type/tax_rate); POS/KDS still HIGH — see `extraction-readiness.md`. Phase 2 COMPLETE; extraction is Phase 3.
