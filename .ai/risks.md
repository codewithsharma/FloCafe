...
- ⚠️ RISK: **Full `npm run lint`** remains red on pre-existing frontend React Compiler rules (`set-state-in-effect`, `refs` in `useKdsConnection.ts`, recipes/QR/TableQrDialog, etc.) — baseline debt; do not conflate with P6/P7/P8/P9.
...
- ⚠️ RISK: **P1.2 REC-01 GREEN WITH HARDENING + startup hotfix** — missing-DB recovery server stays alive (localhost). **R14 closed corrupt-openable latch** (`corrupt_database`). **P1-06 CLOSED (2026-08-15 hardening):** hard-unopenable `new Database()` → `DatabaseRecoveryRequiredError('corrupt_database')` + recovery latch (`tests/p1-06-unopenable-db.test.ts`). Residual: JWT recover UI still limited; no backup `installation_id`; POS-port WS upgrade handler still registered but rejected in recovery; new-machine recover still needs Master PIN. Live DR drill still **NOT CLAIMED**.
