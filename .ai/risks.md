...
- 🔴 DEBT: Extraction readiness: Inventory MEDIUM (writes+ledger+history HTTP; stock columns still on products; write HTTP still product-nested); Tax MEDIUM (HTTP + snapshot contract frozen; denormalized columns + money-path orchestration remain); Product/POS/KDS still HIGH — see `extraction-readiness.md`.
