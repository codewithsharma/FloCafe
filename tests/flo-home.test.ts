...
function main(): void {
  ...
  assert.ok(dashboard.includes('/reports/daily-stats'), 'live stats API preserved for HOME metrics');
  assert.ok(dashboard.includes('href="/reports"') || dashboard.includes("href='/reports'"), 'HOME links to reports');
  assert.ok(dashboard.includes('href="/operations"') || dashboard.includes("href='/operations'"), 'HOME links to operations');
  assert.ok(!dashboard.includes('DayCloseCard'), 'DayCloseCard moved off HOME');
  assert.ok(!dashboard.includes('/reports/topProducts'), 'heavy topProducts analytics moved to Reports');
  ...
  assert.ok(!dashboard.includes('bg-white rounded-xl'), 'legacy card pattern removed from dashboard');
  ...
  assert.ok(fs.existsSync(path.join(FRONTEND, 'components/flo/MetricCard.tsx')), 'MetricCard exists');
  assert.ok(fs.existsSync(path.join(FRONTEND, 'components/flo/AttentionStrip.tsx')), 'AttentionStrip exists');
  ...
  assert.ok(en.includes('"flo.home.openReports"') || en.includes('"flo.home.viewReports"'), 'HOME reports CTA i18n');
  assert.ok(en.includes('"flo.home.openOperations"') || en.includes('"flo.home.viewOperations"'), 'HOME operations CTA i18n');
