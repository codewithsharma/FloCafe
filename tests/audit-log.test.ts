...
async function main() {
  ...
  assert.equal(latestMigration.version, 68, 'latest migration is v68');
  assert.equal(latestMigration.name, 'm3_audit_logs_table');
  ...
  assert.equal(upgraded.pragma('user_version', { simple: true }), 68);
  assert.ok(
