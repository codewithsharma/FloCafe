...
async function main() {
  ...
  const db = initTestDb();
  assertEqual(getSupportedSchemaVersion(), 89, 'R9-EXP schema tip is v89 (post R10–R14 tip)');
  ...
  try {
    ...
    assertEqual(tip, 89, 'schema tip includes INV-AUTO-86 (v89); REAL cutover still deferred');
