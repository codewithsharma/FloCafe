...
Exact graphs are declared in `main/modules/catalog.ts` (soft metadata). Rule: **if module A cannot function without B, A lists B**. Soft diagnostics report missing declared deps; fail-closed remount is Phase 3.1 — [phase-3.1-fail-closed-remount.md](phase-3.1-fail-closed-remount.md).
...
**CURRENT:** soft diagnostics still report (`validateVerticalDependencies`, `validateRegistryIntegrity`). Phase 3.1 fail-closed remount validates composition before mount — [phase-3.1-fail-closed-remount.md](phase-3.1-fail-closed-remount.md).

**TARGET:** runtime vertical selection + production Retail (Phase 3.2+). Do not implement a heavy orchestration framework yet.
