...
const flatRatePackData = require('../../../../tests/fixtures/synthetic-flat-rate-pack.json');
...
(async () => {
  ...
  seedPosFixture();
  // Extra cafe menu for realistic workflows
  const db = getDatabase();
  const t = now();
  ...
  console.log('[QA-E2E] Seeded roles: owner/manager/cashier/waiter/chef + menu');
  await startServer();
