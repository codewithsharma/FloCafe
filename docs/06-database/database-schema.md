...
Schema version **66**. Authoritative sources: `main/db.ts` (`createSchema()`, `MIGRATIONS[]`, helper schema functions).

## Table count: 40 live tables

- **23 tables** created by `createSchema()` on fresh install (migration v1)
- **17 additional tables** added by later migrations and helpers (`createCloudSyncSchema()`, `createWhatsAppSchema()`, etc.)
