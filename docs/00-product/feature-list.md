...
| Printer health / recovery (PRINT-HEALTH) | [BUILT]     | `GET /api/printers/health` + Settings recovery UX; concurrent claim; `npm run test:print-health`                         | High                 |
...
| Day close / Z-report workflow    | [BUILT]     | `main/services/day-close.ts`, Operations UI; cash Z print/download Phase 3.6D; R9 Slice 4 polish (`test:r9.4`, Z txt export audits)         | High                 |
...
| Ops finance reports (R9 Slice 5) | [BUILT]     | `GET /api/reports/ops-finance` + expenses CSV; compose Gross/Net + posted expenses; `tests/r9-ops-finance-reports.test.ts`                  | High                 |
| Food-cost report (R9 Slice 6)    | [BUILT]     | `GET /api/reports/food-cost` theoretical COGS from recipe consumptions; Reports UI; `tests/r9-food-cost-report.test.ts`                     | High                 |
| Void/cancel report (R12)         | [BUILT]     | `GET /api/reports/voids` + `export/voids.csv` from audit_logs; `report.voids_exported`; Reports UI; `tests/r12-void-report.test.ts`         | High                 |
| Payment report (RPT-PAY)         | [BUILT]     | `GET /api/reports/payments` + `export/payments.csv`; tender + refunds by method; `npm run test:rpt-pay`                                     | High                 |
| Discount report (RPT-DISC)       | [BUILT]     | `GET /api/reports/discounts` + `export/discounts.csv`; order+item comps; `npm run test:rpt-disc`                                            | High                 |
| Staff report (RPT-STAFF)         | [BUILT]     | `GET /api/reports/staff` + `export/staff.csv`; creator sales + audit/refund/shift activity; `npm run test:rpt-staff`                        | High                 |
| Product report (RPT-PRODUCT)     | [BUILT]     | `GET /api/reports/products` + `export/products.csv`; settled merchandise + category rollup; `npm run test:rpt-product`                      | High                 |
| Category report (RPT-CATEGORY)   | [BUILT]     | `GET /api/reports/categories` + `export/categories.csv`; projects P11 `by_category`; `npm run test:rpt-category`                            | High                 |
| Data/audit hardening (P13)       | [BUILT]     | In-txn audits for bill discount / order create / item discount / restore; merge + print retry integrity; `npm run test:data-audit`          | High                 |
...
| Offline KDS / KDS recovery (durable outbox) | [BUILT]     | Schema v87 `kds_delivery_outbox`; snapshot delivery intent + FE reconnect backoff (`KDS-H-OUTBOX`)                                                                                           | High                 |
| KDS sound & visual alerts (KDS-ALERTS)      | [BUILT]     | FE `KdsAlertTracker` + header sound toggle; no schema bump; `npm run test:kds-alerts`                                                                                                        | High                 |
| Online ordering                             | [BUILT]     | R10 table QR guest menu + dine_in create; pay-at-counter; `/qr/?t=`; rotate; v84 (`r10-online-qr-ordering.md`)                                                                               | R10 COMPLETE         |
| Coupons / offers                            | [BUILT]     | R11 `coupons` + `/api/coupons` + `POST /api/orders/:id/apply-coupon`; v85 (`r11-coupons.md`)                                                                                                 | R11 COMPLETE         |
