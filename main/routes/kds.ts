...
function getKdsUserHasStationAssignments(
...
function isRestrictedKdsPayload(
  req: Request,
  categoryIds: string[],
  stationIds: string[],
): boolean {
...
function getKdsUserCategoryIds(db: ReturnType<typeof getDatabase>, req: Request): string[] | null {
  ...
  const user = db
    .prepare('SELECT role, category_ids, is_active FROM users WHERE id = ?')
    .get(userId) as
    | {
        role: string;
...
router.get('/orders', requireKdsEnabled, (req: Request, res: Response) => {
  try {
    ...
    if (
      stationId &&
      !db.prepare('SELECT 1 FROM kitchen_stations WHERE id = ? AND is_active = 1').get(stationId)
    ) {
    ...
    const requestedStationCategoryIds = stationId
      ? getKdsStationCategoryIds(db, [stationId])
      : stationCategoryIds;
    ...
    if (stationId) {
      const categoryRoute =
        requestedRoutingCategoryIds.length > 0
    ...
    const allVisibleItems = (orders as any[])
      ...
      .filter(
        (i) =>
          ...
          isKdsStationItemAllowed(
            payloadStationIds,
            requestedRoutingCategoryIds,
            ordersById.get(i.order_id)?.kitchen_station_id,
            i.category_id,
            ordersById.get(i.order_id)?.kitchen_station_id
              ? requestedScope.categoryIdsByStation[
                  String(ordersById.get(i.order_id)?.kitchen_station_id)
                ]
              : undefined,
            requestedScope.hasUnrestrictedStation,
          ),
      );
    ...
    const ordersWithItems = (orders as any[])
      .map((order) => {
          ...
          .filter(
            (i) =>
              ...
              isKdsStationItemAllowed(
                ...
                i.category_id,
                order.kitchen_station_id
                  ? requestedScope.categoryIdsByStation[String(order.kitchen_station_id)]
                  : undefined,
                requestedScope.hasUnrestrictedStation,
              ),
          )
        ...
        return {
          ...projectKdsOrder(order, restrictedPayload),
          items: visibleItems,
...
router.get('/pairing', (req: Request, res: Response) => {
  try {
    ...
    const stations = (
      db
        .prepare('SELECT * FROM kitchen_stations WHERE is_active = 1 ORDER BY sort_order, name')
        .all() as any[]
    )
      .filter((station) => {
        ...
        return (
          ...
          stationCategories.some((categoryId) => userCategoryIds.includes(categoryId))
        );
      })
...
router.post(
  ...
  validateBody(kdsPairingBodySchema),
  (req: Request, res: Response) => {
    try {
      ...
      if (station_id) {
        const station = db
          .prepare('SELECT * FROM kitchen_stations WHERE id = ? AND is_active = 1')
          .get(station_id);
        if (!station) {
          return res.status(404).json({ error: 'Kitchen station not found' });
        }
...
router.get('/display', requireKdsEnabled, (req: Request, res: Response) => {
  try {
    ...
    const stationItemCategoryIds: string[] | null =
      stationCategoryIds.length > 0
        ? userCategoryIds.length > 0
    ...
    const items = attachEffectiveAddons(db, db.prepare(itemsQuery).all(...params) as any[]).map(
      (item) => projectKdsItem(item, restrictedPayload),
    );
    ...
    res.json({
      station: projectKdsStation(
        station,
        isRestrictedKdsPayload(req, userCategoryIds, userStationIds),
        userCategoryIds,
      ),
...
router.patch(
  '/items/:id/status',
  requireKdsEnabled,
  validateBody(kdsItemStatusBodySchema),
  (req: Request, res: Response) => {
    try {
      ...
      if (expectedStatus !== undefined && !validStatuses.includes(expectedStatus)) {
        return res
          .status(400)
          .json({ error: `Invalid expected status. Use: ${validStatuses.join(', ')}` });
      }
      ...
      const updatedItem = withTxn(() => {
        ...
        const currentUser = db
          ...
          .get((req as any).user?.userId) as { tokens_valid_after: string | null } | undefined;
        if (
          !currentCategoryIds ||
          !currentStationIds ||
          ...
          isTokenStale((req as any).user?.iat, currentUser.tokens_valid_after)
        )
        ...
        const currentStationScope = getKdsStationRoutingScope(
          db,
          currentStationIds,
          currentCategoryIds,
        );
      ...
      `,
        ...
        if (userCategoryIds.length > 0) {
          const product = db
            ...
            .get(item.product_id) as { category_id: string | null } | undefined;
        ...
        `,
          ...
          if (
            !isKdsStationItemAllowed(
              currentStationIds,
              currentRoutingCategoryIds,
              station?.kitchen_station_id,
              item.category_id,
              station?.kitchen_station_id
                ? currentStationScope.categoryIdsByStation[String(station?.kitchen_station_id)]
                : undefined,
              currentStationScope.hasUnrestrictedStation,
            )
          ) {
            throw new Error('STATION_FORBIDDEN');
          }
        ...
        applyKitchenItemStatus(db, {
          itemId: routeParam(req.params.id),
          status,
          expectedStatus,
          actorUserId: (req as any).user?.userId,
        });

        return db.prepare('SELECT * FROM order_items WHERE id = ?').get(routeParam(req.params.id));
      });
      ...
      notifyKdsUpdate();
      res.json({ item: projectKdsItem(updatedItem, restrictedPayload) });
    } catch (error: unknown) {
      if ((error instanceof Error ? error.message : String(error)) === 'VOIDED_ITEM') {
        return res
          .status(400)
          .json({ error: 'This item has been voided and can no longer be updated' });
      }
      if (
        (error instanceof Error ? error.message : String(error)) === 'USER_FORBIDDEN' ||
        (error instanceof Error ? error.message : String(error)) === 'PERMISSIONS_UNAVAILABLE'
      ) {
        return res.status(403).json({ error: 'Could not load current station permissions' });
      }
      if (
        (error instanceof Error ? error.message : String(error)) === 'CATEGORY_FORBIDDEN' ||
        (error instanceof Error ? error.message : String(error)) === 'STATION_FORBIDDEN'
      ) {
      ...
      if (
        (error instanceof Error ? error.message : String(error)) === 'STATUS_CONFLICT' ||
        (error instanceof KitchenStatusError &&
          (error as { code?: string }).code === 'STATUS_CONFLICT')
      ) {
    ...
    try {
      ...
      withTxn(() => {
        setOrderKitchenPriority(db, {
          orderId: routeParam(req.params.id),
          priority,
          actorUserId,
        });
      });
      ...
      const order = db
        ...
        .get(routeParam(req.params.id)) as any;
