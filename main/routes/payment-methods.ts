...
function normalizeName(value: unknown): string {
  ...
  if (['cash', 'card', 'loyalty', 'loyalty wallet', 'wallet'].includes(name.toLowerCase())) {
    throw Object.assign(new Error('That name is reserved by a built-in payment method'), {
      statusCode: 409,
    });
  }
  return name;
...
router.get('/merge-history', requireRole('owner', 'manager'), (_req, res) => {
  res.json({
    merges: getDatabase()
      .prepare('SELECT * FROM payment_method_merges ORDER BY merged_at DESC, id DESC LIMIT 30')
      .all(),
  });
});
...
router.get(
  ...
  (req: Request, res: Response) => {
    const includeInactive =
      ...
      ['owner', 'manager'].includes((req as any).user.role);
...
router.post(
  '/',
  validateBody(paymentMethodCreateBodySchema),
  ...
  (req: Request, res: Response) => {
    try {
      ...
      const max = db
        .prepare('SELECT COALESCE(MAX(sort_order), 0) AS n FROM payment_methods')
        .get() as { n: number };
      const result = db
        .prepare(
          'INSERT INTO payment_methods (name, is_active, sort_order, created_at, updated_at) VALUES (?, 1, ?, ?, ?)',
        )
        .run(name, Number(max.n) + 10, now(), now());
      void sendEvent('feature_used', { feature: 'custom_payment_methods', action: 'added' });
      res.status(201).json({
        payment_method: list(true).find((row) => row.id === Number(result.lastInsertRowid)),
      });
    } catch (error: any) {
...
router.put(
  ...
  validateBody(paymentMethodUpdateBodySchema),
  (req: Request, res: Response) => {
    try {
      ...
      const active =
        req.body?.is_active === undefined ? current.is_active : req.body.is_active ? 1 : 0;
      const order =
        req.body?.sort_order === undefined ? current.sort_order : Number(req.body.sort_order);
      if (!Number.isSafeInteger(order) || order < 0)
        return res.status(400).json({ error: 'Invalid sort order' });
      db.prepare(
        'UPDATE payment_methods SET name = ?, is_active = ?, sort_order = ?, updated_at = ? WHERE id = ?',
      ).run(name, active, order, now(), id);
      if (active !== current.is_active)
        void sendEvent('feature_used', {
          feature: 'custom_payment_methods',
      ...
      res.status(duplicate ? 409 : error.statusCode || 500).json({
        error: duplicate
          ? 'A payment method with this name already exists'
          : error.message || 'Unable to update payment method',
      });
    }
  },
);
...
router.delete('/:id', requireRole('owner', 'manager'), (req: Request, res: Response) => {
  ...
  if (used)
    return res.status(409).json({
      error: `This method has ${used} recorded payment${used === 1 ? '' : 's'}. Disable or merge it instead.`,
    });
...
router.post(
  ...
  requireRole('owner', 'manager'),
  validateParams(paymentMethodIdParamsSchema),
  validateBody(paymentMethodMergeBodySchema),
  (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        (targetType === 'custom' && (!Number.isSafeInteger(targetId) || sourceId === targetId))
      ) {
      ...
      const result = withTxn(() => {
        ...
        const target =
          targetType === 'card'
            ? null
            : (db.prepare('SELECT * FROM payment_methods WHERE id = ?').get(targetId) as any);
        ...
        const update = db.prepare(
          'UPDATE bills SET payment_details = ?, updated_at = ? WHERE id = ?',
        );
        let affected = 0;
        for (const row of rows) {
          ...
          if (changed)
            update.run(JSON.stringify(Array.isArray(parsed) ? lines : lines[0]), now(), row.id);
        }
        const sourceKey = `custom:${sourceId}`;
        const targetKey = targetType === 'card' ? 'card' : `custom:${targetId}`;
        const collision = db
          .prepare(
            `SELECT 1 FROM payment_transaction_refs s JOIN payment_transaction_refs t ON t.method = ? AND t.transaction_id = s.transaction_id WHERE s.method = ? LIMIT 1`,
          )
          .get(targetKey, sourceKey);
        if (collision)
          throw Object.assign(
            new Error(
              'The methods contain the same transaction reference and cannot be merged automatically',
            ),
            { statusCode: 409 },
          );
        db.prepare('UPDATE payment_transaction_refs SET method = ? WHERE method = ?').run(
          targetKey,
          sourceKey,
        );
        db.prepare(
          'INSERT INTO payment_method_merges (source_name, target_name, affected_payments, merged_at) VALUES (?, ?, ?, ?)',
      ...
      res
        ...
        .json({ error: error.message || 'Unable to merge payment methods' });
    }
  },
);
