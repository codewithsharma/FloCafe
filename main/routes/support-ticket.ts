...
function supportProfile(req: Request) {
  ...
  const currentUser = userId
    ? (db
        .prepare('SELECT name, email, role FROM users WHERE id = ? AND is_active = 1')
        .get(userId) as SupportUser | undefined)
    : null;
  const owner = db
    .prepare(
...
router.get(
  ...
  (req: Request, res: Response) => {
    ...
    const row = getDatabase()
      ...
      .get(clientTicketId) as
      { status: string; support_code: string | null; last_error: string | null } | undefined;
...
router.post('/', requireRole(...supportRoles), async (req: Request, res: Response) => {
  ...
  const clientTicketId =
    typeof body.client_ticket_id === 'string' && CLIENT_TICKET_ID_RE.test(body.client_ticket_id)
  ...
  const severity = ALLOWED_SEVERITIES.has(String(body.severity || ''))
    ...
    : 'normal';
  const profile = supportProfile(req);
  const contactEmail = String(body.contact_email || profile.contact_email)
    .trim()
    .slice(0, 255);
  ...
  const queued = await cloudSync.queueSupportTicket({
    ...
    contact_name:
      String(body.contact_name || profile.contact_name)
        .trim()
        .slice(0, 255) || undefined,
    contact_email: contactEmail || undefined,
    contact_phone:
      String(body.contact_phone || profile.contact_phone)
        .trim()
  ...
  res.status(queued.queued ? 202 : 503).json({
    ...
    message: queued.queued
      ? 'Your request is queued and will be sent when Operavia is online.'
