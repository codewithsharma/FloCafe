...
export const DRIVE_BACKUP_FOLDER_NAME = 'Nexora Backups';
...
class GoogleDriveService {
  ...
  async connect(): Promise<GoogleDriveStatus> {
    ...
    if (!tokens.refresh_token) {
      ...
      throw new Error('Google did not return a refresh token. Revoke Nexora access at myaccount.google.com/permissions and try connecting again.');
    }
      ...
      const server = http.createServer((req, res) => {
        ...
        res.end(
          error || !code || returnedState !== state
            ? '<html><body>Google Drive connection failed. You can close this window and try again in Nexora.</body></html>'
            : '<html><body>Google Drive connected. You can close this window and return to Nexora.</body></html>'
        );
